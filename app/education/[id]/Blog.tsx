...
import { randomBlogs } from "@/data/blogs";
...
const Blog: React.FC<BlogPageProps> = ({ params }) => {
	const [relatedBlogs, setRelatedBlogs] = useState(() => {
		...
		return randomBlogsResult
			.filter((blog) => blog.id !== Number(params.id))
			.slice(0, 3);
	});
			...
			<div className="container max-w-4xl mx-auto px-4 py-12">
				<article className="prose prose-lg md:prose-xl max-w-none prose-headings:text-gray-800 prose-p:text-gray-700 prose-blockquote:border-l-emerald-500 prose-blockquote:text-gray-700 prose-a:text-emerald-600 prose-a:no-underline hover:prose-a:text-emerald-700">
					...
					{!blog.content?.intro && (
						<>
							...
							<p className="mb-8">
								Electronic waste, commonly known as e-waste, represents one of
								the fastest-growing waste streams globally. With rapid
								technological advancements and the decreasing lifespan of
								electronic devices, proper management of e-waste has become a
					...
					{!blog.content?.sections && (
						<>
							...
							<p className="mb-6">
								E-waste consists of discarded electronic products such as
								computers, televisions, smartphones, and household appliances.
								These devices contain valuable materials like gold, silver,
								copper, and palladium, but also harmful substances including
							...
							<p className="mb-6">
								Improper disposal of e-waste leads to serious environmental
								consequences. When electronic devices end up in landfills, toxic
								materials can leach into soil and groundwater, contaminating
								ecosystems and posing health risks to living organisms.
							</p>
							...
							<p className="mb-6">
								Proper e-waste recycling involves specialized processes to
								safely extract valuable materials while containing hazardous
								components. Certified e-waste recycling facilities employ
								advanced techniques to dismantle electronics, separate
								materials, and process them for reuse.
							</p>

							<blockquote className="border-l-4 border-emerald-500 pl-4 italic my-8">
								&ldquo;The average smartphone contains about 60 different
								elements &ndash; including precious metals and rare earth
								elements &ndash; that could be recovered and reused if the
								device is properly recycled.&rdquo;
							</blockquote>
							...
							<p className="mb-6">
								Consumers play a crucial role in addressing the e-waste
								challenge. By extending the lifespan of devices through proper
								maintenance, choosing products with eco-friendly designs, and
								ensuring responsible disposal through certified recycling
								centers, individuals can significantly reduce e-waste&apos;s
				...
				<div className="bg-emerald-50 border border-emerald-100 rounded-lg p-6 my-12">
					...
					<p className="text-gray-700 mb-4">
						Properly disposing of your electronic waste is easier than you
						think. Find certified e-waste recycling facilities near you and
						ensure your devices are handled responsibly.
					</p>
					<Link
						href="/e-facilities"
