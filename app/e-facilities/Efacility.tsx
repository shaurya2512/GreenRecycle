...
const FacilityMap: React.FC = () => {
	...
	const directionsPopupRef = useRef<mapboxgl.Popup | null>(null);
	...
	useEffect(() => {
		if (clientLocation) {
			...
			geocoder.on(
                ...
                <p class="text-sm text-gray-700">Address: ${
									place_name || "Address not available"
								}</p>
              </div>`
						);
          ...
          <h3 class="font-bold text-indigo-600 text-lg mb-1">Your Location</h3>
			...
			sortedFacilities.forEach((facility, index) => {
				const popup = new Popup().setHTML(
					`<div class="p-3">
            <h3 class="font-bold text-emerald-600 text-xl mb-2">${
							facility.name
						}</h3>
		...
		try {
			...
			if (data.code === "Ok" && mapRef.current) {
				const drawRoute = () => {
					...
					data.routes[0].geometry.coordinates.forEach(
						(coord: [number, number]) => bounds.extend(coord)
					);
              ...
              <p class="text-md">Estimated time: <span class="font-semibold">${durationInMinutes} minutes</span></p>
				...
				if (!mapRef.current!.isStyleLoaded()) {
					mapRef.current!.once("styledata", drawRoute);
				} else {
					drawRoute();
				}
			}
	...
	return (
		<>
			<Head>
				...
				<meta
					name="description"
					content="Locate certified e-waste recycling facilities near you. Get directions, facility details, and book recycling services with our interactive map."
				/>
			</Head>
				...
				{isLoading ? (
					<div className="flex items-center justify-center h-screen">
						<div className="text-center">
							<div className="w-16 h-16 mx-auto mb-4 border-4 rounded-full border-emerald-500 border-t-transparent animate-spin"></div>
											...
											>
												...
												<div className="mb-3 space-y-1 text-gray-600">
													<div className="flex items-start">
														<FaMapMarkerAlt className="flex-shrink-0 mt-1 mr-2 text-gray-400" />
														<p>{info.address}</p>
													</div>
													<div className="flex items-center">
														<FaPhoneAlt className="flex-shrink-0 mr-2 text-gray-400" />
														<a
															href={`tel:${info.phone}`}
												...
												<div className="flex space-x-2">
													<button
														className="flex items-center justify-center flex-1 px-4 py-2 text-sm font-medium text-white transition-colors bg-indigo-500 rounded-md hover:bg-indigo-600"
														onClick={(e) => {
															e.stopPropagation();
															if (clientLocation) {
													...
													<Link
														href="/recycle"
														className="flex items-center justify-center flex-1 px-4 py-2 text-sm font-medium text-white transition-colors rounded-md bg-emerald-500 hover:bg-emerald-600"
													>
					...
					<div className="flex items-center justify-center h-screen px-4 bg-gray-50">
						<div className="max-w-md mx-auto text-center">
							...
							<button
								...
								className="px-6 py-3 font-medium text-white transition-colors duration-300 rounded-md bg-emerald-500 hover:bg-emerald-600"
							>
